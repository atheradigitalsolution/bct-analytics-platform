...
def _landing_columns(wh, table: str) -> dict:
    """The landing table's own columns, read from the warehouse.

    NOT from Odoo. `source_columns()` asks an Odoo database what a table looks like, which is
    exactly the question a non-Odoo client cannot answer — they have no Odoo. The landing table
    already exists and already carries the shape the policy produced, so it is the honest source
    of truth here, and it keeps this command working for a client whose application we never see.

    Bookkeeping columns (`_op`, `_tenant_id`, `_lsn`, `_ingested_at`, `_row_id`) are excluded:
    they are written by the loader, never by a file.
    """
    with wh.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute(
            "SELECT a.attname AS column_name, "
...
def cmd_load_csv(args) -> int:
    """Land a client's CSV into `raw.*` through the same policy the CDC loader obeys.

    WHY THIS EXISTS. `import-policy` classifies a non-Odoo client's columns and then stops —
    it writes `warehouse.column_policy` and lands nothing. Everything after that point already
    existed for Odoo sources and for the FDW fixture, so the gap was never the pipeline; it was
    one missing entry point into it.

    WHAT IS DELIBERATELY SHARED WITH `load-fixture`. The masking expression (`_mask_expr`), the
    `_op='I'` / `_lsn='0/0'::pg_lsn` landing convention, and the `pipeline_state` write are the
    same code and the same values. A second landing path that masked differently, or ordered
    differently, would be a second definition of what `raw.*` means — and the first symptom would
    be a client's personal data landing in clear because one of the two paths was never updated.

    WHY A TYPED STAGING TABLE AND NOT A DIRECT INSERT. `_mask_expr` is SQL over a relation, so it
    needs a relation to read. Staging also puts the type conversion in Postgres rather than in
    Python: a CSV holds only strings, and deciding in Python that "00123" is an integer or that
    "2026-13-01" is a date means re-implementing a parser that the database already has and that
    already rejects the things it should reject.

    COLUMNS THAT HASH ARE STAGED AS `text`. Their landing column is `text` (a 64-char digest) but
    their SOURCE value is whatever the client wrote, so staging them with the landing type would
    reject the input before it could ever be hashed.

    FAIL-CLOSED, IDENTICALLY TO THE LOADER. A column in the file with no policy row is a hard
    refusal with exit code 3 — the same code the CDC loader uses for the same reason. Contract 01:
    unclassified data must not land, and defaulting it to `public` is never the answer.
    """
    path = pathlib.Path(args.file)
    if not path.is_file():
        print(f"no such file: {path}", file=sys.stderr)
    ...
    # --- the tenant, and its salt -----------------------------------------------------------
    tenant = next((t for t in tenants(wh) if t["tenant_id"] == args.tenant), None)
    if tenant is None:
        known = ", ".join(t["tenant_id"] for t in tenants(wh)) or "(none)"
        print(
            f"FATAL: tenant {args.tenant!r} is not an active row in warehouse.tenant_registry. "
            f"Known active tenants: {known}.",
            file=sys.stderr,
        )
        return 2
    salt = salt_for(tenant)

    # --- the landing table has to exist already ----------------------------------------------
    landing = _landing_columns(wh, args.table)
    if not landing:
        print(
            f"FATAL: raw.{args.table} does not exist or has no data columns.\n"
            f"       Landing DDL is generated by `make warehouse-raw-ddl`, which today derives\n"
            f"       column types from an Odoo database and only covers SOURCE_TABLES. A table\n"
            f"       that exists in no Odoo has no generator yet — that gap is real and this\n"
            f"       command does not paper over it by inventing types from the file.",
            file=sys.stderr,
        )
        return 2
    ...
    with wh.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        ...
        pol = {r["source_column"]: dict(r) for r in cur.fetchall()}
    ...
    if not header:
        ...
        return 2
    if not records:
        print(f"{path} contained no data rows", file=sys.stderr)
        return 2

    unclassified, unknown_to_landing = [], []
    load_cols, dropped = [], []
    for name in header:
        p = pol.get(name)
        if p is None:
            ...
            continue
        if p["transform"] == "drop":
            # `secret`. It has no column in the landing table at all, so there is nothing to
            # write it to even if someone wanted to. Reported, not silently skipped.
            dropped.append(f"{name} ({p['pdp_class']})")
            continue
        if name not in landing:
            unknown_to_landing.append(name)
            continue
        load_cols.append(name)

    if unclassified:
        print(
            f"\nFATAL: {len(unclassified)} column(s) in {path} have no row in "
            f"warehouse.column_policy for source_table={args.table!r}:",
            file=sys.stderr,
        )
        for c in unclassified:
            print(f"  - {c}", file=sys.stderr)
        print(
            "\nContract 01: an unclassified column is a hard failure, never a silent default to\n"
            "`public`. Classify them first:  make import-policy FILE=policies/<client>.csv",
            file=sys.stderr,
        )
        return 3
    if unknown_to_landing:
        print(
            f"\nFATAL: {len(unknown_to_landing)} classified column(s) have no column in "
            f"raw.{args.table}:",
            file=sys.stderr,
        )
        for c in unknown_to_landing:
            print(f"  - {c}", file=sys.stderr)
        print(
            "\nThe policy and the landing table disagree. Regenerate the landing DDL "
            "(`make warehouse-raw-ddl`) rather than dropping the column here.",
            file=sys.stderr,
        )
        return 3
    if not load_cols:
        print(
            f"FATAL: every column in {path} is classified `secret` and dropped; "
            f"there is nothing to land.",
            file=sys.stderr,
        )
        return 2

    print(f"==> {path.name}: {len(records)} rows, {len(load_cols)} column(s) to land")
    if dropped:
        print(f"    dropped by policy: {', '.join(dropped)}")
    ...
    # --- stage, mask, land ----------------------------------------------------------------
    # A hashing column keeps the client's own value in staging; everything else is staged with
    # the landing type so Postgres does the conversion and rejects what it should reject.
    def stage_type(name: str) -> str:
        p = pol[name]
        if p["transform"].startswith("hmac") and not p["mask_null"]:
            return "text"
        return landing[name]["col_type"]

    stage_ddl = ", ".join(f'"{c}" {stage_type(c)}' for c in load_cols)
    quoted = ", ".join(f'"{c}"' for c in load_cols)
    exprs = ", ".join(_mask_expr(landing[c], pol[c]) for c in load_cols)

    rows = [
        tuple((rec.get(c) if (rec.get(c) or "").strip() != "" else None) for c in load_cols)
        for rec in records
    ]

    with wh.cursor() as cur:
        cur.execute(f"CREATE TEMP TABLE csv_stage ({stage_ddl}) ON COMMIT DROP")
        psycopg2.extras.execute_values(
            cur, f"INSERT INTO csv_stage ({quoted}) VALUES %s", rows, page_size=500,
        )
        cur.execute(
            f"INSERT INTO raw.{args.table} ({quoted}, _op, _tenant_id, _lsn) "
            f"SELECT {exprs}, 'I', %(tenant)s, '0/0'::pg_lsn FROM csv_stage",
            {"salt": salt, "tenant": args.tenant},
        )
        n = cur.rowcount

        # Same write, same guard, same reasoning as load-fixture: a run that landed nothing is
        # not evidence that data arrived, and `last_row_at` exists precisely to keep those two
        # apart.
        cur.execute(
            "INSERT INTO warehouse.pipeline_state "
            "  (tenant_id, source_table, last_success_at, last_row_at, rows_loaded, slot_name) "
            "VALUES (%s, %s, now(), CASE WHEN %s > 0 THEN now() END, %s, NULL) "
            "ON CONFLICT (tenant_id, source_table) DO UPDATE SET "
            "  last_success_at = now(), "
            "  last_row_at = COALESCE(EXCLUDED.last_row_at, "
            ...
            "  rows_loaded = warehouse.pipeline_state.rows_loaded + EXCLUDED.rows_loaded, "
            "  last_error = NULL, failure_count = 0",
            (args.tenant, args.table, n, n),
        )
    wh.commit()
    print(f"==> raw.{args.table:<22} {n:>6} rows landed for tenant {args.tenant}")
    return 0
...
def main() -> int:
    ...
    p = sub.add_parser(
        "load-csv",
        help="Land a client's CSV into raw.* through warehouse.column_policy",
    )
    p.add_argument("--file", required=True, help="CSV file; header names must match the policy")
    p.add_argument("--tenant", required=True, help="tenant_id, must be active in the registry")
    p.add_argument("--table", required=True, help="source_table, must exist as raw.<table>")
    p.add_argument("--dry-run", action="store_true",
                   help="Validate the file against the policy and write nothing")
